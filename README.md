...
- `/` 家长页：粘贴课堂总结、预览拆词结果、保存复习日；底部「学习设置」可开自动朗读并选 1/2/3 次。
- `/play` 孩子页：大卡片听读，顶栏左侧「家长」按钮回家长页；播放按钮点一下朗读、再点停止；顶部进度条显示还剩多少。开启自动朗读后，切卡自动发音。

背景色是暖奶油色（`#fff3dc`），对儿童更友好。图标是「字母积木 A + 发音气泡」，PNG 由 `scripts/generate-icons.py` 生成（iOS 主屏幕、Android maskable 均已覆盖）。

...
空状态（还没有任何复习日）时，文本框上方显示「载入示例课」：把内置示例课（`src/data/sampleLesson.ts`，动物 + 颜色 + 数字主题，含教师噪声行）填入文本框，走正常的预览 → 保存流程，不跳过任何步骤。

第一版不提供编辑已有复习日、删除单条词句、改中文释义。可以删除整份复习日：列表项上的删除需二次确认，允许删到零。删光后「给孩子听」禁用。

全局背景色为淡蓝色 `#cfe3f7`；卡片、列表保持白底。
...
- 音标：单词类条目在英文下方显示美式 IPA（如 `/ˈæpəl/`），数据来自内置词表 `src/data/reviewPhonetics.ts`（约 190 个幼儿常见词，查表命中才显示，含简单复数回退）。句子和词表外的词不显示音标；离线不接任何 API，后续如需全覆盖再评估内置离线词典。
- 中文：音标下方。没有中文则不渲染该行。
...
- 句子没有中文时，用内置句型翻译补齐（`src/data/sentencePatterns.ts`）：固定句表（如 `What do you see?`）+ 15 个句型模板（`I see a ___` / `The ___ is ___` / `I have ___` 等）+ 约 240 词离线词表，形容词 + 名词组合加「的」（`blue bird` → 蓝色的鸟），数词 + 名词组合加「个」。命不中模板的句子保持无翻译——离线不接 API，宁可没有也不要错翻。
